...
var initControllerPath = argv.ic || process.env.SOCKETCLUSTER;
...
if (workerControllerPath) {
    ...
    var startWhenFileIsReady = function startWhenFileIsReady(filePath) {
    ...
    var filesReadyPromises = [startWhenFileIsReady(masterControllerPath), startWhenFileIsReady(workerControllerPath), startWhenFileIsReady(brokerControllerPath), startWhenFileIsReady(initControllerPath)];
    Q.all(filesReadyPromises).then(function () {
        start();
    });
} else {
    start();
